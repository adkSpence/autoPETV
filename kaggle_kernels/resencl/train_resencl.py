"""
Kaggle kernel: ResEncL architecture ablation
...
subprocess.run(
    ["nnUNetv2_plan_and_preprocess", "-d", str(DATASET_ID), "-pl", "nnUNetPlannerResEncL",
     "--verify_dataset_integrity", "-c", CONFIGURATION, "-np", "2"],
    check=True,
)

print("=== Training (ResEncL) ===", flush=True)
subprocess.run(
